...
def cli_parse():
    ...
    parser.add_argument(
        "-v", "--verbosity", help="Verbosity level (1, 2, 3, 4, 5)", default=2, type=int
    )
...
def gui_launch():
    """Launch the GUI mode if there are no args or issues reading with given args."""
    exit()


def one_hour_mode(sdate: str, stime: str):
    """
    Generate end date and time from the starting date and time.

    :param sdate:   Start date argument.
